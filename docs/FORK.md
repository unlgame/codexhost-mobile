...
然后让 GitHub Actions 产出属于你自己的 APK。
...
| 内容 | 对应 Secret |
| --- | --- |
| `base64 -w0 codexhost-mobile.keystore` 的输出 | `ANDROID_KEYSTORE_BASE64` |
| 你设的 store 口令 | `ANDROID_KEYSTORE_PASSWORD` |
| `-alias` 的值 | `ANDROID_KEY_ALIAS` |
| 你设的 key 口令 | `ANDROID_KEY_PASSWORD` |
...
3. 依次添加 `ANDROID_KEYSTORE_BASE64`、`ANDROID_KEYSTORE_PASSWORD`、
...
| 「无法用 ANDROID_KEYSTORE_PASSWORD 打开 keystore」 | store 口令填错，注意大小写 |
| 「keystore 中不存在别名 …」 | `ANDROID_KEY_ALIAS` 与 `-alias` 不一致 |
